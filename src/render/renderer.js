...
function redraw(state) {
  ...
  // Track same-species count for color diversity
  const speciesCount = {};
  inoculations.forEach(cfu => {
    const name = cfu.bacteria ? cfu.bacteria.name : 'unknown';
  ...
  inoculations.forEach(cfu => {
    ...
    const bacName = b.name || 'unknown';
    speciesIdx[bacName] = (speciesIdx[bacName] || 0) + 1;
    const sameIdx = speciesIdx[bacName] - 1; // 0=first, 1=second, etc.
    const totalSame = speciesCount[bacName] || 1;
    ...
    // timeRatio'yu envHistory kümülatif ortalamasından türet
    let timeRatio;
    const tIdx = Math.round(time);
    const envHist = state.envHistory;
    if (envHist && tIdx > 0) {
      let sum = 0;
      let count = 0;
      for (let i = 0; i <= tIdx; i++) {
        sum += (envHist[i] !== undefined ? envHist[i] : envFactor);
        count++;
      }
      const avgEnv = sum / count;
      timeRatio = rawRatio * avgEnv;
    } else {
      timeRatio = rawRatio * envFactor;
    }

    let color = interpColor(b.colorStops, timeRatio);
    // Colony diversity: second+ colony of same species gets darker/lighter
    if (sameIdx > 0) {
      const offset = sameIdx * 20;
      color = color.map((c,i) => {
        const shift = i === 0 ? -offset : i === 1 ? -offset*0.5 : offset*0.3;
        return Math.max(30, Math.min(255, c + shift));
      });
    }
